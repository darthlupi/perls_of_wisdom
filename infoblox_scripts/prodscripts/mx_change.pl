		...
		if ( @mx_results )
		  {
				...
				if ( $results->exchanger() =~ m/s5a1.psmtp.com/ )
				  {
					$change_mx = $results->zone() . ".mx1.hargray.rcimx.net";
				  	$change_priority = "10";	
				  } 
				elsif ( $results->exchanger() =~ m/s5a2.psmtp.com/ )
				  {
                                        $change_mx = $results->zone() . ".mx2.hargray.rcimx.net";
					$change_priority = "20";
                                  }
                                elsif ( $results->exchanger() =~ m/s5b1.psmtp.com/ )
                                  {
                                        $change_mx = $results->zone() . ".mx3.hargray.rcimx.net";
					$change_priority = "30";
                                  }
                                elsif ( $results->exchanger() =~ m/s5b2.psmtp.com/ )
                                  {
                                        $change_mx = $results->zone() . ".mx4.hargray.rcimx.net";
					$change_priority = "40";
                                  }
				else
				  {
